                                  ...
                                  target_ulong *entrylo0, target_ulong *entrylo1)
{
    ...
    *entrylo0 = ldl_phys(cs->as, ptw_phys) >> linux_pte_info.softshift;
    *entrylo1 = ldl_phys(cs->as, ptw_phys + 4) >> linux_pte_info.softshift;
}
                                  ...
                                  target_ulong *entrylo0, target_ulong *entrylo1)
{
    ...
    if (linux_pte_info.config & KERNEL_RIXI) {
        ...
        *entrylo1 = (*entrylo1 & 0x3FFFFFFF) |
        ...
        /* Get the entrylo values from pgt */
        *entrylo0 = ldl_phys(cs->as, ptw_phys) >> linux_pte_info.softshift;
        *entrylo1 = ldl_phys(cs->as, ptw_phys + 8) >> linux_pte_info.softshift;
    }
}
                                      ...
                                      int mmu_idx, int is_softmmu)
{
    ...
    linux_pte_info.pagetable_walk(cs, pgd_addr, fault_addr, &entrylo0, &entrylo1);
...
hwaddr mips_cpu_get_phys_page_debug(CPUState *cs, vaddr addr)
{
    ...
    if (ret != TLBRET_MATCH) {
        ...
        if (likely(pgd_addr)) {
            ...
            linux_pte_info.pagetable_walk(cs, pgd_addr, addr, &entrylo0, &entrylo1);
